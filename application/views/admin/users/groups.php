				...
				<?php if (count($groups) != '') foreach ($groups as $group) { ?>
						<tr>
							<td class="center">
								<div class="checkbox-table">
									<label>
										<?php if($group->default == 0){ ?>
											<input type="checkbox" name="checkb[]" class="checkb" value="<?php echo $group->id; ?>">
										<?php }else{ ?>
											<input type="checkbox" class="checkb" value="" disabled="true">
										<?php } ?>
									</label>
								</div>
							</td>
							...
							<td class="center">
								<div class="visible-md visible-lg hidden-sm hidden-xs">
									...
									<?php if($group->default == 0){ ?>
										<a class="remove btn btn-bricky tooltips" data-placement="top" data-original-title="<?php echo lang('remove');?>" href="<?php echo site_url(); ?>admin/users/deletegroup/<?php echo $group->id; ?>" onclick="return confirm('<?php echo lang('user_confirm_delete_group');?>');">
											<i class="fa fa-times"></i>
										</a>
									<?php }else{ ?>
										<a class="remove btn btn-default">
											<i class="fa fa-times"></i>
										</a>
									<?php } ?>
								</div>
