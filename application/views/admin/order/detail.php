...
<div id="order_detail_body">
	<div class="row">
								...
								<div class="order_detail">
									...
									<div class="row">
										...
										<span class="col-sm-7 text-left">
											<a href="<?php echo site_url('admin/users/edit/'.$order->user_id); ?>" title="<?php echo $order->name; ?>">
									...
									<div class="row">
										...
										<span class="col-sm-7 text-left">
											<a href="<?php echo site_url('admin/users/edit/'.$order->user_id); ?>" title="<?php echo $order->username; ?>">
		...
		<div class="col-md-3">
			...
			<h4><?php echo lang('orders_admin_orders_note_title');?></h4>
			<form method="POST" action="<?php echo site_url('orders/detail/'.$order->id); ?>">
				<div class="form-group">
					<textarea id="order_note" name="note" class="form-control" rows="4" placeholder="<?php echo lang('orders_admin_orders_note_title');?>"></textarea>
				</div>
				<div class="form-group">
					<button type="button" class="btn btn-primary pull-right" onclick="addNote();"><?php echo lang('save');?></button>
				</div>
			</form>
		</div>
	</div>
...
<script type="text/javascript">
	...
	function addNote()
	{
		var val = jQuery('#order_note').val();
		jQuery.ajax({
			type: "POST",
			url: '<?php echo site_url('admin/orders/detail/'.$order->id); ?>',
			dataType: 'html',
			data: {note : val},
			success: function(data){
				if(data != '')
				...
				{
					alert('<?php echo lang('order_admin_save_note_error_msg'); ?>');
				}
			}
		});
	}
