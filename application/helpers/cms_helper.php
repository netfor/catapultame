...
function getBreadcrumbCate($cate_id, $url = 'categories/', $default = true, $bread = array())
{
	$breadcrumbs = getParentCate($cate_id);
	if(count($breadcrumbs))
	{
		$i = 0;
		foreach($breadcrumbs as $key=>$val)
		{
			$key = $key.'-'.url_title(strtolower($val));
			if($i == 0 && $default == true)
				$href = 'javascript:void(0)';
			else
				$href = site_url($url.$key);
...
function getParentCate($cate_id, $categories = array())
{
	$CI =& get_instance();
	$CI->db->where('id', $cate_id);
	...
	if(count($category))
	{
		...
		if($category->parent_id > 0)
			$categories = getParentCate($category->parent_id, $categories);
	}
	return $categories;
}
...
function getLanguages()
{
	...
	if(file_exists($path_lang))
		return @parse_ini_file($path_lang);
	else
...
function getCss($data, $type = '')
{
	...
	if (isset($params->background->image) && $params->background->image != '') $background .= 'background-image:url("'.base_url($params->background->image).'");';
	if (isset($params->background->image) && $params->background->image != '') $background .= 'background-image:url("'.base_url($params->background->image).'");';
	if (isset($params->background->style) && $params->background->style != '' && $params->background->style != 'Defaults' && $params->background->style != 'No repeat') $background .= 'background-repeat:'.$params->background->style.';';
	if (isset($params->background->style) && $params->background->style != '' && $params->background->style == 'No repeat') $background .= 'background-repeat: no-repeat;';
	...
	else
	{
		...
		$css .= '.module-'.$elm.' .module-padding{'.$background.'}';
...
function getEmail($email = '')
{
	...
	$query = $CI->db->get('settings');
	$data = $query->row();
	$setting = json_decode($data->settings);
...
function dispayListCate($categories, $level = 0, $link = 'categories/', $show = 'no', $module_id = '')
{
	if (!is_array($categories) OR empty($categories)) return '';
	...
	if(count($categories))
	{
		$cate_id = array();
		$string = uri_string();
		if($link == 'categories/' && strpos($string, 'roduct/') > 0)
		{
			$id = (int)str_replace('product/', '', $string);
			$CI =& get_instance();
			$CI->load->model('product_m');
			$catedata = $CI->product_m->getProductCate($id, false);
		}
		
		if($link == 'blog/category/' && strpos($string, 'post/') > 0)
		{
			$id = (int)str_replace('blog/post/', '', $string);
			$CI =& get_instance();
			$CI->db->where('id', $id);
			$query = $CI->db->get('article');
			$catedata = $query->row();
		}
		
		if(empty($catedata->cate_id))
		{
			$catedata = new stdClass();
			$catedata->cate_id = (int)str_replace($link, '', $string);
		}
		
		foreach($categories as $category)
		{
			if($category->id == $catedata->cate_id)
			{
				$icon = '<span class="glyphicon glyphicon-minus" onclick="list_cate_toggle_'.$module_id.'(this);"></span>';
				$style = 'style="display: block"';
				$active = 'class="active"';
			}else
			{
				$icon = '<span class="glyphicon glyphicon-plus" onclick="list_cate_toggle_'.$module_id.'(this);"></span>';
			...
			if($level == 0)
			{
				...
				if(count($category->subcategories) > 0 && $show == 'no')
				{
					$html .= '<ul '.$style.' class="nav nav-list">';
					$html .= dispayListCate($category->subcategories, $level, $link, $show);
					$html .= '</ul>';
				}
				$html .= '</li>';
			}
	...
	if(count($categories))
	{
		...
		foreach($categories as $category)
		{
			...
			if($show == 'yes')
			{
				if($level == 0)
				{
					$html .= '<li><a href="'.site_url().$link.$category->id.'-'.$category->slug.'" style="display: block;"><img '.$style.' class="img-thumbnail" src="'.$category->image.'" alt="'.$category->title.'"/></a></li>';
				}
