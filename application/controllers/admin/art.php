	...
	function remove()
	{
		...
		if($ids > 0)
		{
			foreach($ids as $id)
			{
				...
				if($this->art_m->remove($id))
				{
					if(count($art))
					{
						$path = ROOTPATH .DS. 'media' .DS. 'cliparts'.DS.$art->cate_id.DS.'print'.DS.$art->file_name;
						$medium = ROOTPATH .DS. 'media' .DS. 'cliparts'.DS.$art->cate_id.DS.'medium'.DS. md5($art->clipart_id.'medium').'.png';
						$large = ROOTPATH .DS. 'media' .DS. 'cliparts'.DS.$art->cate_id.DS.'large'.DS. md5($art->clipart_id.'large').'.png';
	...
	function edit($id = null)
	{
		...
		if(count($art) > 3)
		{
			...
			if( !class_exists("Imagick") ) {
				$this->session->set_flashdata('error', 'Sorry, You can not add clipart because your server not support PHP Imagick. Please install it!');
				redirect('admin/art');
			}
			...
			if (isset($_FILES["file"]["name"]) && $_FILES["file"]["name"] != '')
			{
				...
				if($clipart_id > 0)
				{
					$clipart = $this->art_m->getArt($clipart_id);
					if(isset($clipart->file_name))
					{	
						$path = ROOTPATH .DS. 'media' .DS. 'cliparts'.DS. $clipart->cate_id .DS. 'print' .DS. $clipart->file_name;
						if(file_exists($path))
							unlink($path);
					}
				}
			...
			$art['slug']	= url_title(strtolower($art['slug']));
